...
def MakePlan():
    plan_file = "/tmp/mstress_%s_%s.plan" % (
        getpass.getuser(),
        time.strftime("%F-%H-%M-%S", time.gmtime()),
    )
    ...
    subprocess.call(
        [
            ...
            Params.CLIENT_HOSTS,
            "--clients-per-host",
            str(Params.CLIENTS_PER_HOST),
            "--path-type",
            Params.PATH_TYPE,
            "--levels",
            str(Params.PATH_LEVELS),
            "--inodes-per-level",
            str(Params.INODES_PER_LEVEL),
            "--num-to-stat",
            str(Params.NumFiles2Stat()),
            "-o",
            plan_file,
        ]
    )
