...
MetaAllocate::start()
{
    if (suspended || layoutDone) {
        panic("invalid allocate start invocation");
        return false;
    }
...
MetaAllocate::LayoutDone(int64_t chunkAllocProcessTime)
{
    ...
    if (0 == status) {
        ...
        if (wasSuspended) {
            ...
            submit_request(this); // resume.
        }
    } else {
        const bool kCountAllocTimeFlag = true;
        LogDone(wasSuspended, kCountAllocTimeFlag, chunkAllocProcessTime);
    }
}

void
MetaAllocate::LogDone(
    bool resumeFlag, bool countAllocTimeFlag, int64_t chunkAllocProcessTime)
{
    ...
    if (0 <= status && pendingLeaseRelinquish) {
        assert(! pendingLeaseRelinquish->clnt);
...
MetaAllocate::logOrLeaseRelinquishDone(int code, void* data)
{
    ...
    if (this == data) {
        ...
        if (this != origClnt) {
            ...
            LogDone(kResumeFlag, kCountAllocTimeFlag, kAllocTime);
            return 0;
        }
...
submit_request(MetaRequest* r)
{
    ...
    if (r->submitCount++ == 0) {
        ...
        if (MetaRequest::kLogNever != r->logAction) {
            ...
            if (logFlag && r->suspended) {
                panic("invalid write ahead log suspended request");
            }
            if (logFlag || r->suspended) {
