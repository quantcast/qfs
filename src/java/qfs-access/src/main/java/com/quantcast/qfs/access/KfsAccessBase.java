...
class KfsAccessBase {
    ...
    private static native long initF(String configFn);

    private static native long initS(String metaServerHost, int metaServerPort);

    final protected static native void destroy(long ptr);

    private static native int cd(long ptr, String path);

    private static native int mkdir(long ptr, String path, int mode);

    private static native int mkdirs(long ptr, String path, int mode);

    private static native int rmdir(long ptr, String path);

    private static native int rmdirs(long ptr, String path);

    private static native String[] readdir(
            long ptr, String path, boolean prefetchAttr);

    private static native String[][] getDataLocation(
            long ptr, String path, long start, long len);

    private static native String[][] getBlocksLocation(
            long ptr, String path, long start, long len);

    private static native short getReplication(long ptr, String path);

    private static native short setReplication(
            long ptr, String path, int numReplicas);

    private static native long getModificationTime(
            long ptr, String path);

    private static native int create(
            long ptr, String path, int numReplicas, boolean exclusive,
            int numStripes, int numRecoveryStripes, int stripeSize,
            int stripedType, boolean forceType, int mode, int minSTier,
            int maxSTier);

    private static native int create2(
            long ptr, String path, boolean exclusive, String createParams);

    private static native int create2ex(
            long ptr, String path, boolean exclusive, String createParams,
            int mode, boolean forceTypeFlag);

    private static native int remove(long ptr, String path);

    private static native int rename(
            long ptr, String oldpath, String newpath, boolean overwrite);

    private static native int symlink(
            long ptr, String target, String linkpath, int mode,
            boolean overwrite);

    private static native int open(
            long ptr, String path, String mode, int numReplicas,
            int numStripes, int numRecoveryStripes, int stripeSize,
            int stripedType, int createMode);

    private static native int exists(long ptr, String path);

    private static native int isFile(long ptr, String path);

    private static native int isDirectory(long ptr, String path);

    private static native long filesize(long ptr, String path);

    private static native long setDefaultIoBufferSize(long ptr, long size);

    private static native long getDefaultIoBufferSize(long ptr);

    private static native long setDefaultReadAheadSize(long ptr, long size);

    private static native long getDefaultReadAheadSize(long ptr);

    private static native long setIoBufferSize(long ptr, int fd, long size);

    private static native long getIoBufferSize(long ptr, int fd);

    private static native long setReadAheadSize(long ptr, int fd, long size);

    private static native long getReadAheadSize(long ptr, int fd);

    private static native int setUTimes(
            long ptr, String path, long mtime_usec, long atime_usec,
            long ctime_usec);

    private static native int compareChunkReplicas(long ptr, String path, StringBuffer md5sum);

    // private static native int getStripedType(long ptr, String path);
    private static native void setFileAttributeRevalidateTime(
            long ptr, int secs);

    private static native int chmod(long ptr, String path, int mode);

    private static native int chmodr(long ptr, String path, int mode);

    private static native int fchmod(long ptr, int fd, int mode);

    private static native int chowns(
            long ptr, String path, String user, String group);

    private static native int chownsr(
            long ptr, String path, String user, String group);

    private static native int chown(
            long ptr, String path, long user, long group);

    private static native int chownr(
            long ptr, String path, long user, long group);

    private static native int fchowns(
            long ptr, int fd, String user, String group);

    private static native int fchown(long ptr, int fd, long user, long group);

    private static native int setEUserAndEGroup(
            long ptr, long user, long group, long[] groups);

    private static native int stat(long ptr, String path, KfsFileAttr attr);

    private static native int lstat(long ptr, String path, KfsFileAttr attr);

    private static native String strerror(long ptr, int err);

    private static native boolean isnotfound(long ptr, int err);

    private static native int close(long ptr, int fd);

    private static native long seek(long ptr, int fd, long offset);

    private static native long tell(long ptr, int fd);

    private static native int setUMask(long ptr, int umask);

    private static native int getUMask(long ptr);

    private static native String createDelegationToken(
            long ptr, boolean allowDelegationFlag, long validTime,
            KfsDelegation result);

    private static native String renewDelegationToken(
            long ptr, KfsDelegation token);

    private static native String cancelDelegationToken(
            long ptr, KfsDelegation token);
    ...
    static {
        try {
            ...
            throw new RuntimeException(
                    "Unable to load qfs_access native library", e);
        }
    }
    ...
    final public KfsDelegation kfs_createDelegationToken(
        ...
        final String error = createDelegationToken(
                cPtr, allowDelegationFlag, validTime, result);
        if (error != null) {
    ...
    final public Map<String, String> kfs_getStats() throws IOException {
        ...
        if (stats.length % 2 != 0) {
            throw new IOException(
                    "internal error: invalid stats array size: "
                    + stats.length);
        }
