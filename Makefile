...
# Absolute path to the krb.env file generated by setup_kerberos.sh or
# src/test-scripts/krb/krb-test.sh.  Required when running the krbtest target.
KRB_ENV_FILE=
...
.PHONY: krbtest
krbtest: hadoop-jars python
	if [ x"${KRB_ENV_FILE}" != x ]; then \
		cd build/${BUILD_TYPE} && \
		../../src/test-scripts/qfstest.sh \
			`${QFS_PYTHON_TEST_OPTION}` \
			-install-prefix . \
			-kerberos "${KRB_ENV_FILE}" \
			${QFSTEST_OPTIONS} ; \
	else \
		echo 'kerberos environment not set, skipping kerberos tests'; \
	fi

.PHONY: mintest
mintest: hadoop-jars python krbtest
	cd build/${BUILD_TYPE} && \
	../../src/test-scripts/qfstest.sh \
		`${QFS_PYTHON_TEST_OPTION}` \
