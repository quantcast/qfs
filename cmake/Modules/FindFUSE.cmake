...
if(FUSE_FOUND)
    foreach(FUSE_VERSION_INCLUDE
        ...
        if(EXISTS "${FUSE_VERSION_INCLUDE}")
            ...
            string(
                REGEX REPLACE
                ".*# *define *FUSE_MAJOR_VERSION *([0-9]+).*"
                "\\1" FUSE_MAJOR_VERSION "${_contents}"
            )
            string(
                REGEX REPLACE
                ".*# *define *FUSE_MINOR_VERSION *([0-9]+).*"
                "\\1" FUSE_MINOR_VERSION "${_contents}"
            )
...
int main(void) {
    ...
    if(NOT FUSE_CFLAGS_CHECK)
        ...
        # Should we run again previous test to assume the failure was due to
